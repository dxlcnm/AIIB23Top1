...
def postprocess(root, save_root, name):
    # for name in name_list[:]:
    print('post processing on:', name)
    image = sitk.ReadImage(os.path.join(root, name, 'pred.nii.gz'))
    ...
    sitk.WriteImage(image_save, os.path.join(save_root, name, 'pred.nii.gz'), useCompression=True)
...
def multiprocess():
    root = "/data/data2/datasets/ctLung/Airway/nifti_new/"
    save_root = root
    name_list = pd.read_csv("/data/data1/trainname.csv")['filename'].tolist()
    name_list.sort()
    p = Pool(8)
    for f in name_list:
        p.apply_async(postprocess, (root, save_root, f))
    p.close()
    p.join()
