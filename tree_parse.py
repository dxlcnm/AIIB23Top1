...
def tree_parse_tw(ids, case_name, label_root, save_root):
    
    f = case_name
    if not os.path.isfile(os.path.join(save_root, f)):
        create_directories(os.path.join(save_root, f))
        sitk_label = sitk.ReadImage(os.path.join(label_root, f, "airway_merge.nii.gz"))
        label=sitk.GetArrayFromImage(sitk_label).transpose(2, 1, 0)
        ...
        skeleton = skeletonize_3d(label)
        print(f"skeleton time: {time.time()-time1}s")
        skeleton_sitk = sitk.GetImageFromArray(skeleton.transpose(2, 1, 0))
        skeleton_sitk.CopyInformation(sitk_label)
        sitk.WriteImage(skeleton_sitk, os.path.join(label_root, f, "skeleton_merge.nii.gz"), useCompression=True)
        ...
        while whether_refinement(parent_map, children_map, tree_parsing, num, trachea) is True:
            ...
            parent_map, children_map, generation = parent_children_map(ad_matric, trachea, num)
        time3 = time.time()
        print(f"parsing time: {time3-time2}s")
        print(ids, f, "finished!")
        tree_parsing_sitk = sitk.GetImageFromArray(tree_parsing.transpose(2, 1, 0))
        tree_parsing_sitk.CopyInformation(sitk_label)
        sitk.WriteImage(tree_parsing_sitk, os.path.join(save_root, f, "parsing_merge.nii.gz"), useCompression=True)
        # parse_nii = nibabel.Nifti1Image(tree_parsing, np.eye(4))
        # nibabel.save(parse_nii, os.path.join(save_root, f))
def tree_parse_tw_process():
    label_root = "/data/data2/datasets/AIIB2023/AIIB23_Train_T1_use_all_trainset/AIIB23_Train_T1/nifti/"
    file_path = './data/base_dict_aiib2023.json'
    save_root = "/data/data2/datasets/AIIB2023/AIIB23_Train_T1_use_all_trainset/AIIB23_Train_T1/nifti/"
    file_list = load_train_file(file_path, folder='0', mode=['train', 'val'])
    file_list.sort()
    print((f"file list len: {len(file_list)}"))

    num_pool = 6
    # p = Pool(num_pool)
    for idx, case_name in enumerate(file_list):
        if case_name !="AIIB23_101":
            continue
        # print(idx)
        tree_parse_tw(idx, case_name, label_root, save_root)
    #     p.apply_async(tree_parse_tw, (idx, case_name, label_root, save_root))
    # p.close()
    # p.join()

if __name__ == '__main__':
    tree_parse_tw_process()